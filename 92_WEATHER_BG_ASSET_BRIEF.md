...
> **必須用「現有棚景背景圖」當參考圖（image-to-image / 局部重繪），不要純文字生成。**
> 純文字會生出構圖/透視/物件位置都不同的棚 → 切換時兩張對不上、會晃。
> 做法：把現有的晴天棚景餵給 GPT，指令是「**保持整體構圖、透視、所有室內物件位置 100% 不變，只改①窗外景色 ②整體採光/氛圍 ③地板上從窗戶照進來的光**」。
