...
**🎨 男主持陳柏偉也改半寫實全身（2026-06-23、✅ 全套上線）+ 去背 bug 修復**：比照女主持、**17 張表情全換半寫實全身**（brief `101_MALE_HOST_SEMIREALISTIC_BRIEF.md`、人設沿用草根熱血+深藍襯衫/深灰褲/黑皮鞋、眼睛看鏡頭）。新增專屬 `去背工具_陳柏偉.bat`（對齊 `assets/char_3q/emo_idle.png`、不會誤對到女主持）。使用者調好定位（`aming` xRatio 0.33 / yOffsetFromWall 470 / `scale.characterIndividualAming` 0.56）、舊圖備份在 `assets/char_3q/OLD_backup/`。**兩位主持現在都半寫實、風格一致**。**⚠️ 順帶修了 `char_keyer.py` 去背 bug**：原本綠幕判定靠「離綠色歐氏距離 <120」當主判據 → 深藍/深色衣服距離也 <120 被誤刪出破洞、頭從脖子斷開被 `keep_largest` 丟掉（女主持淺色洋裝沒中、男主持深藍襯衫中招）。改成**主要看 greenness（綠色是否明顯主導：綠幕≈+96、深藍 -32 分超開）**、dist 只當輔助小門檻 → 深色衣服安全、兩位都正常。工具也加：男/女各一個拖放 bat、印出「對齊哪個角色」標籤。
**🎨 女主持改半寫實全身 AI 人像 + 去背工具（2026-06-23、✅ 全套上線）**：王于安（內部 id 仍 legacy `xiaomei`）從動漫風換成**半寫實全身人像**。① 生圖規格 + 定版 prompt → `100_FEMALE_HOST_SEMIREALISTIC_BRIEF.md`（全身 head-to-toe、**1254×1254 正方形人物置中左右留透明**、**眼睛直視鏡頭**、綠幕底、不像真實名人；棚是開放式無桌、兩位主持都全身站姿）。② **新增去背工具**：`tools/char_keyer.py` + 根目錄 `去背工具.bat`（拖放即用）+ `tools/去背工具_說明.md`。功能＝綠幕圖 → 去綠幕 + 清 `+` 對位標記（最大連通塊）+ 等比置中對齊到現有 `emo_idle` 的腳/高度 → 輸出可直接覆蓋的透明 1254² PNG（需 numpy/PIL、清雜點需 scipy、本機都已裝；輸出檔名=輸入檔名、可批次處理整個資料夾）。③ 服裝定版為**及膝連身洋裝**、眼睛直視鏡頭。④ **15 張表情/動作全部換成新半寫實版**（emo_* 12 + act_* 3）：使用者用 `去背工具.bat` 批次去背 + 自行調好 config 定位（`hosts.xiaomei.yOffsetFromWall` 490、`scale.characterIndividual` 0.56、`aming.xRatio` 0.37 等）→ **已正常上線運作**（舊 idle 備份在 `emo_idle_OLD_backup.png`）。⑤ OfficeScene 站位計算 + config.js 位置欄位已補中文註解（角色 sprite 錨點在腳底、`yOffsetFromWall` 越大越往下）。**之後可考慮**男主持陳柏偉（id `aming`、在 `assets/char_3q/`）也改半寫實、否則兩人風格不搭；目前男主持仍動漫風。
