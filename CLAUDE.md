...
**🎨 女主持改半寫實全身 AI 人像 + 去背工具（2026-06-23、進行中）**：王于安（內部 id 仍 legacy `xiaomei`）從動漫風換成**半寫實全身人像**。① 生圖規格 + 定版 prompt → `100_FEMALE_HOST_SEMIREALISTIC_BRIEF.md`（全身 head-to-toe、**1254×1254 正方形人物置中左右留透明**、**眼睛直視鏡頭**、綠幕底、不像真實名人；棚是開放式無桌、兩位主持都全身站姿）。② **新增去背工具**：`tools/char_keyer.py` + 根目錄 `去背工具.bat`（拖放即用）+ `tools/去背工具_說明.md`。功能＝綠幕圖 → 去綠幕 + 清 `+` 對位標記（最大連通塊）+ 等比置中對齊到現有 `emo_idle` 的腳/高度 → 輸出可直接覆蓋的透明 1254² PNG（需 numpy/PIL、清雜點需 scipy、本機都已裝；輸出檔名=輸入檔名、可批次處理整個資料夾）。③ `emo_idle.png` 已試裝新 master（舊的備份成 `emo_idle_OLD_backup.png`）。**待**：用定版 prompt（眼睛看鏡頭版）生其餘 14 張表情 → 拖進工具批次處理 → 覆蓋 `assets/char_xiaomei/` → 重新整理。**之後可考慮**男主持陳柏偉（id `aming`、在 `assets/char_3q/`）也改半寫實、否則兩人風格不搭；目前男主持仍動漫風。
