...
> ⚠️ **本節所有 tone 一律只輸出 2 行**（JSON 陣列長度 = 2）、陳柏偉與王于安各一行、誰先誰後依 tone。
> **每行 text 固定包含 2 句話**（用句號或問號分開，各 20~30 字），不多不少。

- **debate**：陳柏偉先說觀點（2 句），王于安反嗆收尾（2 句）。觀點針對『現象/制度/規律』、不針對個人或政黨。
- **react**：王于安先提問再驚嘆（2 句），陳柏偉認真分析（2 句）。分析基於新聞事實、不臆測動機。
- **monologue**：陳柏偉連說 2 句分析，王于安 2 句簡短回應結尾。
- **casual**：隨機誰先說都行，輕鬆閒聊，每人各 2 句。
- **critical**：兩人各 2 句批評 topic 的『結構問題 / 制度設計 / 套路』、用『問題在...』『重點是...』『應該要...』。❌ 不要批評特定人 / 政黨 / 公司。✅ 批評現象本身、批評年年炸的規律。
- **mocking**：兩人各 2 句嘲笑 topic 的『荒謬處 / 套路 / 重複歷史』、用『真的假的』『靠夭喔』『甘有可能』語氣。❌ 不嘲笑個人 / 政黨 / 受害者。✅ 嘲笑現象 / 規律 / 制度的荒謬。
- **humorous**：兩人各 2 句用幽默梗 / 玩笑話討論 topic 現象、輕鬆有趣不失分析。❌ 不開特定人玩笑。✅ 開現象 / 套路 / 結構的玩笑。
- **sarcastic**：兩人各 2 句用反諷語氣（『以前不是這樣』『不意外』『所以呢』『唉』），表面平靜實則在嘴 topic 的『規律 / 結構 / 套路』。❌ 不反諷特定人 / 政黨。✅ 反諷『十年前就這樣演、十年後還是這樣』。

## 生成規則

### 字數
- 每行 **包含 2 句話**，每句 **20~30 字**（句與句之間用句號或問號分開）
- 每行合計約 40~60 字
- 句尾自然口語結束
...
- **每輪 2 句中至少有一句**明確提到 topic 或上方關鍵字、或具體引用 topic 背景
...
⚠️ **陣列長度固定 2**（剛好 2 句、陳柏偉與王于安各一句）。不要多、不要少。
...
完整範例（每行 2 句、JSON 陣列長度固定 2）：
[
  {{"speaker": "aming", "text": "這個結構年年炸、靠夭喔。說真的啦、問題就在這個制度設計！", "emotions": ["humor", "angry"]}},
  {{"speaker": "xiaomei", "text": "不會吧、這也太誇張了。所以呢、每次出事最後還是消費者買單？", "emotions": ["surprised", "skeptical"]}}
]"""
...
async def generate_chat():
    ...
    try:
        ...
        # Step 5.33: 每輪固定 2 句（陳柏偉、王于安各一句）。模型偶爾多吐 → 硬截到前 2 句。
