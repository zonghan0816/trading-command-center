...
## 11. 「沒聲音」根因 + 重試修復（2026-06-05 晚、重要）

部署後使用者反映「還有沒聲音的問題」（log 常見 `TTS 2/4`、`1/3`）。實測診斷：

- YunJhe **循序呼叫 10/10 全成功** → 不是這個聲音永久壞掉。
- 一輪 4 句結果常 3/4 或更差 → edge-tts **「間歇性」回空音訊**（單次約 15~40%、隨機、跟平行/循序關係不大）。
- 早上的全掛是 YunJhe 當時真的壞、之後已自己恢復；現在的零星沒聲音是這個「隨機間歇失敗」。

**之前熔斷器設計對「間歇失敗」是錯的**：一句失敗就讓整個聲音靜音 10 分鐘（連坐後面所有句），反而製造更多沒聲音。

修復：

| 機制 | 做法 |
|---|---|
| **每句重試** | `_TTS_RETRY=4`、退避遞增（0.5→1.0→1.5s）、跨過 edge-tts 爛掉的幾秒。單句失敗率 15% → 重試 4 次後 ~0.05% |
| **連續失敗才算掛** | `_tts_fail_streak[speaker]`：連續 `_TTS_DOWN_THRESHOLD=4` 句都重試全失敗才 `_mark_voice_down`（靜音+梗）。單句隨機失敗只讓那句靜音、不連坐、遇到成功歸零 |
| **不互相連坐** | 每位主持人各自 streak、一位掛掉不影響另一位 |

**實測**：retry 版連跑 5 輪 fresh（清快取重生）全 4/4、streak 全程 0、不誤觸 cooldown；模擬真的掛（無效聲音）→ 連續失敗到門檻才觸發梗、王于安全程有聲不受連坐。→ 「沒聲音」問題解決。
